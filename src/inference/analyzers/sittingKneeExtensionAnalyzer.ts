...
export class SittingKneeExtensionAnalyzer extends BaseAnalyzer {
  ...
  analyze(_landmarks: Landmark[], landmarkHistory: Landmark[][] = []): TestResult {
    ...
    try {
      ...
      const pelvicStability = this.evaluatePelvicStability(leftHip, rightHip);
  ...
  private evaluateKneeExtensionCompleteness(kneeAngle: number): number {
    ...
    if (kneeAngle >= 160) {
      ...
      return 70 + (kneeAngle - 140) * 1.5;
    } else if (kneeAngle >= 120) {
      return 40 + (kneeAngle - 120) * 1.5;
    } else {
      return Math.max(0, kneeAngle * 0.33);
    }
  }
  ...
  private evaluatePelvicStability(leftHip: Landmark, rightHip: Landmark): number {
    // 骨盤の水平安定性評価
